...
import { map, repeat, zipWith, concat, chain , pair } from "ramda";
...
import { Tuple, SExpValue, isTuple, isClosure, makeClosure, Closure, Value, makeTuple } from "./L5-value";
...
import { getVarDecs, getTupless, getSexp, sameLen } from "./L5-typecheck";
...
export const evaLetValues = (exp: LetvaluesExp, env: Env): Result<Value> => {
        const varsD : VarDecl[][] = map((b: BindingValues) =>
         b.vars, exp.bindings);
    
        const valsTemp: Result<SExpValue[]> = mapResult((v : CExp) =>
        applicativeEval(v, env), map((b : BindingValues) => b.val, exp.bindings));
    
        const vars = getVarDecs(varsD);
        const valTup: Result<SExpValue[][]> = bind(valsTemp, (val: SExpValue[]) => mapResult(getTupless, val));
        const allvals = bind(valTup,(valofT:SExpValue[][]) => makeOk(getSexp(valofT)));
    
        const isequal:Result<boolean> = safe2((vard: VarDecl[][], vals: SExpValue[][]) =>
         makeOk(sameLen(vard,vals)))
        (makeOk(varsD),valTup);
        return isOk(isequal)?
            isequal.value?
        bind(allvals, (vals:SExpValue[]) => evalSequence(exp.body, makeExtEnv(vars, vals, env))):
        makeFailure("length incorrect"):
        makeFailure("err");
}
