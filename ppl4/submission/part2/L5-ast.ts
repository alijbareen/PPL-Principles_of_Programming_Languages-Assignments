...
export const parseValuesExp = (sexp: Sexp[]): Result<ValuesExp> =>
    isArray(sexp)? safe2((val: CExp[]) =>
     makeOk(makeValuesExp(val)))
    (mapResult(parseL5CExp,sexp), makeOk("x") ) :
    makeFailure("Bad Values not array\n");    

//NEW
export const parseLetValuesExp = (binds: Sexp, body: Sexp[]): Result<LetvaluesExp> =>
    isEmpty(body) ? makeFailure('Body of "let-values" cannot be empty') :
    ! isGoodBindingValues(binds) ? makeFailure(`Invalid Values bindings: ${JSON.stringify(binds)}`) :
    safe2((bdgs: BindingValues[], body: CExp[]) =>
     makeOk(makeLetValuesExp(bdgs, body)))
    (parseBindingValues(binds), mapResult(parseL5CExp, body));


//NEW
export const isGoodBindingValues = (binds: Sexp): binds is [Sexp[], Sexp][] =>
    isArray(binds) && allT(isArray, binds) && allT(isArray, map(first, binds));

export const parseBindingValues = (bindings: [Sexp[], Sexp][]): Result<BindingValues[]> =>
...
export const parseVarDeclArr = (arr: Sexp[]):Result<VarDecl[]>=>
...
export const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
...
//NEW
export const unparsevalues = (v: ValuesExp) : Result<string> =>
        bind(unparseLExps(v.val),(vals: string) => makeOk(`(values ${vals})`))
 //NEW       
 export const unparseLetvalues = (lval: LetvaluesExp) : Result<string> => 
    safe2((vars: string, body: string) => makeOk(`(let-values (${vars}) ${body})`))
        (unparseBindingvalues(lval.bindings), unparseLExps(lval.body));
//NEW
export const unparseBindingvalues = (binds: BindingValues[]): Result<string> =>
    bind(
        mapResult(
            x => safe2((v: string, val: string) => 
            makeOk(`(${v} ${val})`))(unparseVarDeclarr(x.vars), unparse(x.val)), binds),
        (bind1: string[]) => makeOk(join(" ", bind1)));
//NEW
export const unparseVarDeclarr = (vard: VarDecl[]): Result<string> =>
    bind(mapResult(unparseVarDecl, vard), unparsed => makeOk(`(${unparsed.join(' ')})`));
