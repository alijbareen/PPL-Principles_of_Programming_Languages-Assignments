...
(define sqrt2
  (lambda (x init epsilon)
   (head (find-first (sqrt-lzl x init)
               (lambda (guess-approx)
                 (good-enough? (head guess-approx) x epsilon))))
  )
)
...
(define collect-all-values-1
 (lambda (lists key)
  (cond ((empty? lists) lists)
        (else (let ((gv (get-value (car lists) key))
                    (gvs (collect-all-values-1 (cdr lists) key)))
                (if (eq? gv 'fail)
                     gvs
                    (cons gv gvs)))))
 )
